...
{
    return "v1.0, 11/04/21, IC v0.411.1, Steam"
}
...
GenericGetValue(UpdateGUI, GUIwindow, LblGUI, GameObject)
{
    ...
    {
        var := g_idle.readstring(GameObject.baseAddress, bytes := 0, GameObject.ValueType, (GameObject.GetOffsets())*)
    }
    else
    {
        var := g_idle.read(GameObject.baseAddress, GameObject.ValueType, (GameObject.GetOffsets())*)
    }
...
GenericGetListValue(UpdateGUI, GUIwindow, LblGUI, GameObject, ItemID, isListAlias := 0, ItemOffset := 1)
{
    ...
    if(GameObject.ValueType == "UTF-16")
    {
        var := g_idle.readstring(GameObject.baseAddress, bytes := 0, GameObject.ValueType, offsets*)
    }
    else
    {
        var := g_idle.read(GameObject.baseAddress, GameObject.ValueType, offsets*)
    }
    if(GameObject.ValueType == "Double" or GameObject.ValueType == "Float")
    {
        var := Round(var, 3)
...
{
    ; var := g_idle.read(g_gameSettings.baseAddress, g_gameSettings.GameSettings.Platform.ValueType, (g_gameSettings.GameSettings.Platform.GetOffsets())*)
    var := GenericGetValue(UpdateGUI, GUIwindow, "ReadGSUserID", g_gameSettings.GameSettings.Hash)
    return var
}